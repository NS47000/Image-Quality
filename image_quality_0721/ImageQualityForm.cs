...
namespace image_quality_0721
{
    public partial class ImageQualityForm : Form
    {
        ...
        public ImageQualityForm()
        {
            ...
            datapath = Path.GetFullPath(System.AppDomain.CurrentDomain.BaseDirectory + @"..\..\..\" + "data");
        ...
        private void loadbutton_Click(object sender, EventArgs e)//讀取圖片資訊，計算後將資料存入TXT中
        {
            ...
            ngdtextBox.Refresh();
            setting config = new setting(6, 8, 5, 50, imagepath, datapath);//設定(相機數量,角度數量,一張圖切成幾成幾,分析sample的最大數量(比實際sample數量多即可),影像位置,存資料位置)
        ...
        {
            setting config = new setting(6, 8, 5, 50, imagepath,datapath);//設定(相機數量,角度數量,一張圖切成幾成幾,影像位置,存資料位置)
        ...
        {
            setting config = new setting(6, 8, 5, 50, imagepath, datapath);
            string textshow = "";//顯示錯誤狀況用的
            float threshold = Convert.ToSingle(textBox1.Text);//讀取閥值參數
            float brightoffset = Convert.ToSingle(brighttextBox.Text);//讀取亮度閥值OFFSET
            float sharpoffset = Convert.ToSingle(sharptextBox.Text);//讀取銳利度閥值OFFSET
        ...
        {
            imagepath = Path.GetFullPath(System.AppDomain.CurrentDomain.BaseDirectory + @"..\..\..\");//設定影像檔案位置
            if (gdcomboBox.SelectedIndex == 0)
            {
                imagepath += "picture\\GD";//golden sample的圖檔資料夾名稱
            }
            else if (gdcomboBox.SelectedIndex == 1)
            {
                imagepath += "picture\\NGD";//normal sample的圖檔資料夾名稱
            }
